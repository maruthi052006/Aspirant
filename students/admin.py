...
from .models import Student, Attendance  # Explicitly import models
...
def export_to_excel(modeladmin, request, queryset):
    ...
    for field in df.columns:
        if pd.api.types.is_datetime64_any_dtype(df[field]):
            df[field] = pd.to_datetime(df[field], errors='coerce')  # Convert to datetime first
    ...
    for field in model._meta.fields:
        if field.get_internal_type() in ["DateField", "DateTimeField"]:
            if field.name in df.columns:
                df[field.name] = pd.to_datetime(df[field.name], errors='coerce')  # Convert if needed
...
# Register models dynamically with export action in Django Admin
class CustomAdmin(admin.ModelAdmin):
    actions = [export_to_excel]

admin.site.register(Student, CustomAdmin)
admin.site.register(Attendance, CustomAdmin)

from django.contrib import admin

admin.site.site_header = "Aspirant admin dashboard"  # Change this to your preferred title
